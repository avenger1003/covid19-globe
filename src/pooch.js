(function () {
  pooch = { version: "0.2.0", author: "Jeremy White" };
  ...
  var __chart = function (id) {
    var _chartScope = this,
      ...
      _mouseIgnore = false,
      _circle = Math.PI * 2,
      _actionObj = { over: "move", out: "move", up: "up", down: "down", move: "move", tap: "tap" },
      _actionsUsed = { over: false, out: false, up: false, down: false, move: false, tap: false },
      _funcQueue = [],
      _symGrp = [],
      _symGrpCur = null,
      _symCur = null,
    ...
    var _fillPop = function (symGrp, sym, id, action, x, y) {
      if (!_symCur || sym[id] !== _symCur[id]) {
        symGrp.action(action).layout(id);
        _movePop(symGrp, action, x, y);
      }
    };

    var _movePop = function (symGrp, action, x, y) {
      var xAdj = 0,
        yAdj = 0,
        xLoc = x - _width,
        yLoc = y - _height,
        popup = symGrp.action(action),
        popOffX = popup.offsetX(),
        popOffY = popup.offsetY(),
        width = popup.width(),
        height = popup.height();
    ...
    var _isPopup = function (symGrp, action) {
      if (symGrp.action(action) && typeof symGrp.action(action).layout === "function") return true;
      return false;
    };
          ...
          if (symFnd) {
            ...
            if (_symCur !== symCur && symCur) {
              ...
              var keyObj = {},
                ...
                showList = ["over", "up", "down", "move", "tap"],
                showLen = showList.length;
              if (_symCur !== null) {
                showList.push("out");
                showLen++;
              }
              ...
              while (showLen--) {
                var showAct = showList[showLen],
                  showObj = _symGrpCur.action(showAct);

                if (showObj && typeof showObj.layout === "function") _fillPop(_symGrpCur, _symGrpCur.state(), _symCur.poochID, showAct, x, y);
                else if (typeof showObj === "function") showObj(_symCur, x, y);
              }
              ...
              var moveList = ["up", "down", "move", "tap"],
                moveLen = moveList.length;

              while (moveLen--) {
                var moveAct = moveList[moveLen],
                  moveObj = _symGrpCur.action(moveAct);

                if (moveObj && typeof moveObj.layout === "function") _movePop(symGrpCur, moveAct, x, y);
                else if (typeof moveObj === "function") moveObj(symCur, x, y);
              }
            }
          }
            ...
            if (_symCur) {
              var hideList = ["over", "out", "up", "down", "move", "tap"],
                hideLen = hideList.length;
              while (hideLen--) {
                var hideAct = hideList[hideLen],
                  hideObj = _symGrpCur.action(hideAct);

                if (hideObj && typeof hideObj.layout === "function") hideObj.hide();
                else if (typeof hideObj === "function") hideObj(_symCur, x, y);
              }
      ...
      var releaseCap = false,
        ...
        drag =
        {
          ...
          start: function (e) {
            ...
            if (window.navigator.msPointerEnabled) // new Microsoft model
            {
              document.addEventListener("MSPointerMove", drag.active, false);
              document.addEventListener("MSPointerUp", drag.end, false);
              document.addEventListener("MSPointerOut", drag.end, false);
            }
            else if (domElem.attachEvent && domElem.setCapture) // old Microsoft model
            {
              releaseCap = true;
              refMove = function () { drag.active(window.event); window.event.returnValue = false; return false; };
              refUp = function () { drag.end(window.event); window.event.returnValue = false; return false; };
              document.attachEvent("onmousemove", refMove);
              document.attachEvent("onmouseup", refUp);
            }
          ...
          active: function (e) {
            ...
            if (drag.elem) {
              ...
              if (offX || offY && !hasMoved) {
                if (_symGrpCur) {
                  var layer = _symGrpCur.layer().toUpperCase(),
                    layerHlt = layer === "MAIN" ? "highlightMain" : "highlightBack",
                    hideList = ["over", "out", "up", "down", "move", "tap"],
                    hideLen = hideList.length;
            ...
            if (releaseCap) {
              ...
              if (domElem.detachEvent) {
                document.detachEvent("onmousemove", refMove);
                document.detachEvent("onmouseup", refUp);
              }
            }
            else {
              ...
              document.removeEventListener("MSPointerMove", drag.active, false);
              document.removeEventListener("MSPointerUp", drag.end, false);
              document.removeEventListener("MSPointerOut", drag.end, false);
            }
            drag.elem = null;
          }
        };
    ...
    _chartScope.symbolGroup = function (obj) {
      ...
      var len = obj.length,
        checkParams = function (e) { var ignore = function () { return _isAnimating || _isDragging || _mouseIgnore || _isZooming; }(); _mouseMoveChart(e, ignore); };
      //_actionObj    = { over: "move", out: "move", up: "up", down: "down", move: "move", tap: "tap" },
      //_actionsUsed  = { over: false, out: false, up: false, down: false, move: false, tap: false },
      ...
      for (var i = 0; i < len; ++i) {
        ...
        if (_house) {
          for (var action in _symGrp[i].action()) {
            if (_symGrp[i].action(action) !== null) {
              if (_isPopup(_symGrp[i], action)) _popHouse(_symGrp[i].action(action).house);

              if (_actionsUsed[action] === false) {
                pooch.fetch("#pooch_mouse_" + _id).action(_actionObj[action], checkParams);
                _actionsUsed[action] = true;
              }
            }
          ...
          //if (_symGrp[i].popup ()) _popHouse (_symGrp[i].action ("move").popup ().house); //_symGrpCur.action ("move") //_symGrp[i].popup ().house
          // if (_isPopup (_symGrp[i], "move")) _popHouse (_symGrp[i].action ("move").house);
          // if (!chartActive && _symGrp[i].interactive ())
          // {
          //   pooch.fetch ("#pooch_mouse_" + _id).action ("mousemove", checkParams) // TODO: Make sure the event isn't getting added more than once
          //                                      .action ("mouseout", checkParams);

          //     var checkParams = function (e) { var ignore = function () { return _isAnimating || _isDragging || _mouseIgnore || _isZooming; } (); func (e, ignore); };
          //     if (_house) pooch.fetch ("#pooch_mouse_" + _id).action ("mousemove", checkParams);
          ...
          //_funcQueue.push ( { func: _chartScope.mouseMove, arg: _mouseMoveChart } ); // TODO: Fix situation missing DOM element
    ...
    _chartScope.action = function (str, func) {
      if (!arguments.length) return _chartScope; // TODO: Does chart need an action object similar to symbolGroup?
      //if (typeof func === "undefined" && typeof str === "string") return _action[str];
      //if (!arguments.length) return _chartScope;
      if (typeof str === "string") pooch.fetch("#pooch_mouse_" + _id).action(str, func);
      else if (typeof str === "object") {
        var i = str.length;
        while (i--) pooch.fetch("#pooch_mouse_" + _id).action(str[i], func); // TODO: Make sure the event isn't getting added more than once
      }
      return _chartScope;
    };
    ...
    _chartScope.zoomControl = function (obj) {
      ...
      _assignDrag(pooch.fetch("#pooch_container_" + _id).dom()); // TODO: See if this should be mouse instead of container
      _zoomControl.update();
      return _chartScope;
    };
    ...
    _chartScope.activeSymbol = function (obj) {
      ...
      if (obj === null) {
        if (_symCur && _symCur.symbolGroup.popup()) _symCur.symbolGroup.popup().hide(); // TODO: Check this to see if it needs to iterate through the action object
      ...
      if (house) {
        ...
        while (ndxPre--) {
          ...
          layout.push("<" + _layersPre[ndxPre][keyPre] + " id='pooch" + keyPre + "_" +
            ...
            _width + "px;height:" + _height + "px;-ms-touch-action:none;'>");
        }
        ...
        _funcQueue = [];
        //if (_house) pooch.fetch ("#pooch_mouse_" + _id).action ("out", function (e) { _mouseMoveChart ({ localX: -20000, localY: -20000 }); });
      }
      return _chartScope;
    };
  ...
  var __symbolGroup = function (shape) {
    ...
    _symGrpScope.action = function (str, func) {
      if (!arguments.length) return _action;
      if (typeof func === "undefined" && typeof str === "string") return _action[str];
      if (typeof str === "string") {
        _action[str] = func;
        if (func && typeof func.layout === "function") _action[str].data(_symGrpScope, _info);
      }
      else if (typeof str === "object") {
        var i = str.length;
        while (i--) {
          _action[str[i]] = func;
          if (func && typeof func.layout === "function") _action[str[i]].data(_symGrpScope, _info);
        }
      }
      return _symGrpScope;
    };
    ...
    var _symGrp = {},
      ...
      //_boundsInView = {},
      _action = { over: null, out: null, up: null, down: null, move: null, tap: null },
      _info = null,
      _pop = null,
      _ctx,
      _order = [],
      ...
      _attrs = {
        symbolGroup: _symGrpScope, x: 0, y: 0, lat: 0, lng: 0, visible: true, shape: "circle",
      ...
      _stepFunc = {
        symbolGroup: _stepSwitch, poochID: _stepSwitch, order: _stepSwitch, x: _stepInt, y: _stepInt,
  ...
  var __zoomControl = function (elem) {
    ...
    _zoomScope.house = function (elem) {
      ...
      if (_reset) pooch.fetch(_reset).action("mousedown", _triggerReset);
      if (_zoomIn) pooch.fetch(_zoomIn).action("mousedown", _triggerZoomIn);
      if (_zoomOut) pooch.fetch(_zoomOut).action("mousedown", _triggerZoomOut);
      if (_handle) pooch.fetch(_handle).action("mousedown", _triggerHandle);
      if (_slider) pooch.fetch(_slider).action("mousedown", _triggerSlider);
  ...
  var __popup = function (elem) {
    ...
    _popupScope.getCovidDetails = function (csym) {
      let country = '';
      let confirmed = '';
      let recovered = '';
      let deaths = '';
      const getReport = `https://covid-19-data.p.rapidapi.com/country/code?format=json&code=${csym}`;
      const getFlag = `https://restcountries.eu/rest/v2/alpha/${csym}`;

      fetch(getReport, {
        "method": "GET",
        "headers": {
          "x-rapidapi-host": "covid-19-data.p.rapidapi.com",
          "x-rapidapi-key": "ed5b68a172msh298cd63cc29bb31p19bff8jsn0f59f339c896"
        }
      })
        .then(checkStatus)
        .then(parseJSON)
        .then(data => {
          // console.log(data)
          covidReport = data[0];
          country = covidReport.country;
          confirmed = covidReport.confirmed;
          recovered = covidReport.recovered;
          deaths = covidReport.deaths;

          /* Get Flag by country and display the markup in popover */
          fetch(getFlag)
          .then(checkStatus)
          .then(parseJSON)
          .then(res => {
            // console.log(res)
            const countryFlag = res.flag;
    
            if (country) {
              _domElem.innerHTML = `
                    <div class="wrapper">
                      <header class="header">${country}</header>
                      <article class="main">
                        <img width="100vw" src="${countryFlag}"/>
                      </article>
                      <aside class="aside aside-1">Confirmed: ${confirmed}</aside>
                      <aside class="aside aside-2">Recoverd: ${recovered}</aside>
                      <footer class="footer">Deaths: ${deaths}</footer>
                    </div>`;
    ...
    _popupScope.layout = function (obj) {
      ...
      _popupScope.hide();
      var response = setTimeout(_popupScope.getCovidDetails(obj), 500);
  ...
  var __fetch = function (elem) {
    ...
    _fetchScope.action = function (str, func) // TODO: Add return actions
    {
      if (!arguments.length) return _fetchScope;

      var newMS = window.navigator.msPointerEnabled,
        oldMS = _domElem.attachEvent && _domElem.setCapture,
        modern = _domElem.addEventListener,
        custEvt = function (e) { _mouseEvent(_domElem, e, func); };

      if (typeof str === "string") {
        switch (str) {
          case "over":
            if (newMS) _domElem.addEventListener("MSPointerOver", custEvt, false);
            else if (oldMS) _domElem.attachEvent("onmouseover", custEvt, false);
            else if (modern) _domElem.addEventListener("mouseover", custEvt, false);
            break;
          case "out":
            if (newMS) _domElem.addEventListener("MSPointerOut", custEvt, false);
            else if (oldMS) _domElem.attachEvent("onmouseout", custEvt, false);
            else if (modern) {
              _domElem.addEventListener("mouseout", custEvt, false);
              _domElem.addEventListener("touchleave", custEvt, false);
              _domElem.addEventListener("touchend", custEvt, false);
              _domElem.addEventListener("touchcancel", custEvt, false);
            }
            break;
          case "up":
            if (newMS) _domElem.addEventListener("MSPointerUp", custEvt, false);
            else if (oldMS) _domElem.attachEvent("onmouseup", custEvt, false);
            else if (modern) {
              _domElem.addEventListener("mouseup", custEvt, false);
              _domElem.addEventListener("touchend", custEvt, false);
            }
            break;
          case "down":
            //_domElem.onmousedown = custEvt;
            if (newMS) {
              _domElem.addEventListener("MSPointerDown", custEvt, false);
              if (typeof _domElem.style.msTouchAction !== "undefined") _domElem.style.msTouchAction = "none";
            }
            else if (oldMS) _domElem.attachEvent("onmousedown", custEvt, false);
            else if (modern) {
              _domElem.addEventListener("touchstart", custEvt, false);
              _domElem.addEventListener("mousedown", custEvt, false);
            }
            break;
          case "move":
            if (newMS) _domElem.addEventListener("MSPointerMove", custEvt, false);
            else if (oldMS) _domElem.attachEvent("onmousemove", custEvt, false);
            else if (modern) {
              _domElem.addEventListener("mousemove", custEvt, false);
              _domElem.addEventListener("touchmove", custEvt, false);
            }
            break;
          case "tap":
            if (newMS) _domElem.addEventListener("MSPointerUp", custEvt, false);
            else if (modern) _domElem.addEventListener("touchstart", custEvt, false);
            break;
          default:
            if (newMS) _domElem.addEventListener(str, custEvt, false);
            else if (oldMS) _domElem.attachEvent(str, custEvt, false);
            else if (modern) _domElem.addEventListener(str, custEvt, false);
            break;
        }
      }
      return _fetchScope;
    ...
    _fetchScope.removeAction = function (str, func) // TODO: Fix this with a unique function reference instead of null
    {
      if (!arguments.length) return _fetchScope;
      if (typeof arr === "string") _domElem.removeEventListener(str, null, false);
      return _fetchScope;
    };
  ...
  var _isSafari = (typeof (navigator.vendor) === "object" && navigator.vendor.indexOf("Apple") !== -1) ? true : false,
    ...
    _mouseEvent = function (domElem, e, func) {
      ...
      if (domElem.getBoundingClientRect) {
        ...
        e.localX = clientX - left;
        e.localY = clientY - top;
        //alert(e.pageX);
      }
      else {
        e.localX = e.clientX || e.pageX; // TODO: Find a better solution for elements without clientRect
        e.localY = e.clientY || e.pageY;
      }
